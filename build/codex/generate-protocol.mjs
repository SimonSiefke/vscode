...
const regexp1 = /\r?\n/;
const regexpZaZ0 = /(\d+\.\d+\.\d+(?:-[A-Za-z0-9.]+)?)/;
const regexp3 = /\r\n/g;
const regexpFrom = /(from\s+["'])(\.\.?\/[^"']+?)(["'])/g;
const regexp5 = /^\s*\n/;
const regexpThisFileWas = /^\/\/ This file was generated by.*$\n/m;
const regexpGENERATEDCODE = /^\/\/ GENERATED CODE!.*$\n/m;
...
function resolveCodexBinary() {
	...
	if (which.status === 0) {
		return which.stdout.trim().split(regexp1)[0];
	}
...
function readBinaryVersion(bin) {
	...
	const m = r.stdout.match(regexpZaZ0);
	if (!m) {
		...
		for (const entry of readdirSync(srcDir, { withFileTypes: true })) {
			...
			if (entry.isDirectory()) {
				...
				const body = readFileSync(src, 'utf8')
					...
					.replace(regexpGENERATEDCODE, '')
					.replace(regexpThisFileWas, '')
					.replace(regexp5, '')
