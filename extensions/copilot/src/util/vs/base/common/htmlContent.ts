...
export function escapeMarkdownSyntaxTokens(text: string): string {
	...
	return text.replace(new RegExp(regexp4), '\\$&'); // CodeQL [SM02383] Backslash is escaped in the character class
}
...
export function escapeMarkdownLinkLabel(text: string): string {
	return text.replace(new RegExp(regexp5), '\\$&');
}
...
export function appendEscapedMarkdownCodeBlockFence(code: string, langId: string) {
	const longestFenceLength =
		code.match(new RegExp(regexp6))?.reduce((a, b) => (a.length > b.length ? a : b)).length ??
		0;
...
export function appendEscapedMarkdownInlineCode(text: string): string {
	const longestBacktickRun = Math.max(0, ...(text.match(new RegExp(regexp7)) ?? []).map(m => m.length));
...
export function escapeDoubleQuotes(input: string) {
	return input.replace(new RegExp(regexp8), '&quot;');
}
...
export function removeMarkdownEscapes(text: string): string {
	...
	return text.replace(new RegExp(regexp9), '$1');
}
