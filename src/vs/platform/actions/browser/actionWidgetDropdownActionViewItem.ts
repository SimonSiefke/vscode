...
export class ActionWidgetDropdownActionViewItem extends BaseActionViewItem {
	...
	private readonly managedHover = this._register(new MutableDisposable());
	constructor(
		action: IAction,
	...
	protected renderLabel(element: HTMLElement): IDisposable | null {
		...
		if (this._action.label) {
			this.managedHover.value = getBaseLayerHoverDelegate().setupManagedHover(this.options.hoverDelegate ?? getDefaultHoverDelegate('mouse'), element, this._action.label);
		} else {
			this.managedHover.clear();
		}
