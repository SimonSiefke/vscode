...
interface SettingIndicator {
	...
	hover?: MutableDisposable<IDisposable>;
}
...
export class SettingsTreeIndicatorsLabel implements IDisposable {
	...
	private createWorkspaceTrustIndicator(): SettingIndicator {
		...
		workspaceTrustLabel.text =
			"$(shield) " +
			localize("workspaceUntrustedLabel", "Requires workspace trust");
	...
	private createPreviewIndicator(): SettingIndicator {
		...
		const hover = disposables.add(new MutableDisposable<IDisposable>());
	...
	private createAdvancedIndicator(): SettingIndicator {
		...
		const advancedLabel = disposables.add(
			new SimpleIconLabel(advancedIndicator),
		);
	...
	private render() {
		...
		const parenthesizedIndicatorsToShow = this.parenthesizedIndicators.filter(
			(indicator) => {
				return indicator.element.style.display !== "none";
			},
		);
		if (parenthesizedIndicatorsToShow.length) {
			...
			DOM.append(
				this.indicatorsContainerElement,
				parenthesizedIndicatorsToShow[parenthesizedIndicatorsToShow.length - 1]
					.element,
			);
			DOM.append(this.indicatorsContainerElement, $("span", undefined, ")"));
		}
	...
	updatePreviewIndicator(element: SettingsTreeSettingElement) {
		...
		this.previewIndicator.hover?.clear();
		...
		if (isPreviewSetting || isExperimentalSetting) {
			const content = isPreviewSetting
				? PREVIEW_INDICATOR_DESCRIPTION
				: EXPERIMENTAL_INDICATOR_DESCRIPTION;
			this.previewIndicator.hover!.value = this.hoverService.setupDelayedHover(
	...
	updateAdvancedIndicator(element: SettingsTreeSettingElement) {
		...
		this.advancedIndicator.element.style.display = isAdvancedSetting
			? "inline"
			: "none";
		this.render();
	}
	...
	updateScopeOverrides(
		...
		if (element.hasPolicyValue) {
			...
			this.scopeOverridesIndicator.label.text =
				"$(briefcase) " +
				localize("policyLabelText", "Managed by organization");
			const content = localize(
				"policyDescription",
						...
						() => ({
							...
							linkHandler: (url: string) => {
								const [scope, language] =
									SettingScopeLink.parse(url).split(":");
								onDidClickOverrideElement.fire({
									settingKey: element.setting.key,
									scope: scope as ScopeString,
									language,
	...
	updateDefaultOverrideIndicator(element: SettingsTreeSettingElement) {
		...
		if (sourceToDisplay !== undefined) {
			...
			this.defaultOverrideIndicator.disposables.add(
				this.hoverService.setupDelayedHover(
	...
	if (defaultValueSource) {
		if (defaultValueSource instanceof Map) {
			...
			for (const [, value] of defaultValueSource) {
				const newValue =
					typeof value !== "string" ? (value.displayName ?? value.id) : value;
